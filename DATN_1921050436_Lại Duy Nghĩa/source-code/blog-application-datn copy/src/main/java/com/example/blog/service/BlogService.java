...
// ============================================================================================================================================================================
    ...
    * */
    ...
    public BlogDto createBlog(UpsertBlogRequest request) {
        ...
        blog.setCreateType(ECreateType.MANUAL);
...
//        }
    ...
    // TODO: lấy 5 bài viết cùng chủ đề
    public List<BlogDto> getBlogsWithSameCate(Integer blogId) {
        Blog blog = blogRepository.findById(blogId).orElseThrow(() -> {
            throw new BadRequestException("Không tìm thấy blog");
        });
        Integer categoryId = blog.getCategory().getId();
        List<BlogDto> result = blogRepository.getBlogsWithSameCate(categoryId, Pageable.ofSize(5));
        return result;
    }
    ...
    // TODO: Lấy ra danh sách tất cả bài viết cho admin
    public Page<BlogDto> getAllBlogsForAdmin(Integer page, Integer pageSize, String sortField, String sortDirection,
                                             String keyword, Integer categoryId, String time) {
        ...
        if (searchStartTime == null) {
            return blogRepository.findAllBlogsForAdmin(keyword, categoryId, PageRequest.of(page - 1, pageSize, parseSortParameter(sortField, sortDirection)));
        }
        Page<BlogDto> searchResult = blogRepository
                .findBlogsForAdminWithTime(keyword, categoryId, searchStartTime, searchEndTime, PageRequest.of(page - 1, pageSize, parseSortParameter(sortField, sortDirection)))
                .map(BlogDto::new);
        return searchResult;
    }

    // TODO: Lấy ra danh sách các bài viết hiển thị trên hệ thống cho admin
    public Page<BlogDto> getAllBlogsDisplayOnWebsiteForAdmin(Integer page, Integer pageSize, String sortField, String sortDirection,
                                             String keyword, Integer categoryId, String time) {
        LocalDateTime searchStartTime = Utils.convertTimeStringToLocalDateTime(time);
        LocalDateTime searchEndTime = LocalDateTime.now();
        if (searchStartTime == null) {
            return blogRepository.findAllBlogsDisplayOnWebsiteForAdmin(keyword, categoryId, PageRequest.of(page - 1, pageSize, parseSortParameter(sortField, sortDirection)));
        }
        Page<BlogDto> searchResult = blogRepository
                .findBlogsDisplayOnWebsiteForAdminWithTime(keyword, categoryId, searchStartTime, searchEndTime, PageRequest.of(page - 1, pageSize, parseSortParameter(sortField, sortDirection)))
                .map(BlogDto::new);
        return searchResult;
    }

    // Tìm kiếm bài viết cho phía client
...
//        List<BlogDto> blogDtoList = blogRepository.findByTag(tag).stream().map(BlogDto::new).collect(Collectors.toList());
        List<BlogDto> blogDtoList = blogRepository.findByTags_Id(tag.getId(), Pageable.ofSize(5)).stream()
            .sorted(Comparator.comparing(Blog::getPublishedAt).reversed())
//                .limit(5)
            .map(blog -> {
                BlogDto blogDto = new BlogDto();
            ...
            .collect(Collectors.toList());
        System.out.println(blogDtoList);
        return blogDtoList;
    }

    public List<BlogDto> getBlogsCreateInAWeekAndHasMostViews() {
        List<BlogDto> result = blogRepository.getBlogsCreateInAWeekAndHasMostViews(LocalDateTime.now().minusWeeks(1), Pageable.ofSize(5))
            .stream().map(blog -> {
                BlogDto blogDto = new BlogDto();
                blogDto.setId(blog.getId());
                blogDto.setTitle(blog.getTitle());
    ...
    // TODO lấy ra danh sách 5 bài viết mới nhất để gửi cho user
    public List<BlogDto> getBlogsToSendForUserReceiveNews() {
        List<BlogDto> result = blogRepository.getBlogsToSendForUserReceiveNews(Pageable.ofSize(5))
                .stream().map(blog -> {
                    BlogDto blogDto = new BlogDto();
                    blogDto.setId(blog.getId());
                    blogDto.setTitle(blog.getTitle());
    ...
    // TODO: Method use for dashboard
//    public Page<BlogDto> getAllBlogsForAdminDashboard(Integer page, Integer pageSize, LocalDateTime from, LocalDateTime to) {
//        Page<Blog> resultSearch = blogRepository.getBlogForDashboard(PageRequest.of(page - 1, pageSize, Sort.by("id").descending()));
////        LocalDateTime searchStartTime = Utils.convertTimeStringToLocalDateTime(time);
////        LocalDateTime searchEndTime = LocalDateTime.now();
////        if (searchStartTime == null) {
////            return blogRepository.findAllBlogsForAdmin(keyword, categoryId, PageRequest.of(page - 1, pageSize, parseSortParameter(sortField, sortDirection)));
