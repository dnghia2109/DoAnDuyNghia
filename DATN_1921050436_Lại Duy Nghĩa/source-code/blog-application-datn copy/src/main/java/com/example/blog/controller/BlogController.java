...
//        Page<BlogDto> pageInfoDto = blogService.getSearchBlogsTest(page, pageSize, sortField, sortDir, keyword, categoryId, time);
    ...
    // TODO: Danh sách bài viết hiển thị trên hệ thống
    @GetMapping("/dashboard/admin/blogs-public")
    public String getBlogsDisplayOnWebsitePage(@RequestParam(required = false, defaultValue = "1") Integer page,
                               @RequestParam(required = false, defaultValue = "10") Integer pageSize,
                               @RequestParam(required = false, defaultValue = "id") String sortField,
                               @RequestParam(required = false, defaultValue = "asc") String sortDir,
                               @RequestParam(required = false) String keyword,
        ...
        List<CategoryDto> categoryList = categoryService.getAllCategories();
        Page<BlogDto> pageInfoDto = blogService.getAllBlogsDisplayOnWebsiteForAdmin(page, pageSize, sortField, sortDir, keyword, categoryIdAsInteger, time);
        String sortReverseDirection = sortDir.equalsIgnoreCase("asc") ? "desc" : "asc";
        model.addAttribute("page1", pageInfoDto);
        model.addAttribute("categoryList", categoryList);
        model.addAttribute("currentPage", page);
