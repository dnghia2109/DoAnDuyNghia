...
// Xử lý sử kiện nút "Chọn hình ảnh"
btnModalImage.addEventListener("click", async () => {
    try {
        // Goi API
        const data = await fetch("/api/v1/files"); // Lấy ra các ảnh đã được đăng bởi user đang login
        const dataJson = await data.json();

        console.log("Data list image của user login")
